#!/bin/bash
# Mobius installer — auto-detects runtime and installs accordingly.
# Usage: curl -fsSL https://raw.githubusercontent.com/tabtoyou/mobius/main/scripts/install.sh | bash
set -euo pipefail

PACKAGE_NAME="mobius-ai"
MIN_PYTHON="3.12"
...
# Ensure mobius binary is in PATH (uv tool install may add to ~/.local/bin)
...
# 4. Setup (mobius CLI configures runtime-specific integration)
if [ -n "$RUNTIME" ] && command -v mobius &>/dev/null; then
  echo
  echo "Running setup..."
  mobius setup --runtime "$RUNTIME" --non-interactive || true
fi
...
if command -v claude &>/dev/null; then
  ...
  # (mobius setup may have done this already, but we ensure it with timeout)
  MCP_FILE="$HOME/.claude/mcp.json"
  mkdir -p "$HOME/.claude"

  # MCP command matches the installer that actually ran in step 3
  if [ "$INSTALL_METHOD" = "uv" ]; then
    MOBIUS_ENTRY='{"command":"uvx","args":["--from","mobius-ai[claude]","mobius","mcp","serve"]}'
  elif [ "$INSTALL_METHOD" = "pipx" ]; then
    MOBIUS_ENTRY='{"command":"mobius","args":["mcp","serve"]}'
  else
    MOBIUS_ENTRY='{"command":"python3","args":["-m","mobius","mcp","serve"]}'
  fi
  ...
  if [ -n "$MCP_PYTHON" ]; then
    if [ -f "$MCP_FILE" ]; then
      if MCP_FILE="$MCP_FILE" MOBIUS_ENTRY="$MOBIUS_ENTRY" $MCP_PYTHON -c "
import json, os
mcp_file = os.environ['MCP_FILE']
entry = json.loads(os.environ['MOBIUS_ENTRY'])
with open(mcp_file) as f:
    data = json.load(f)
servers = data.setdefault('mcpServers', {})
servers['mobius'] = entry
...
" 2>/dev/null; then
  ...
  # 5b. Install/update Mobius skills (claude plugin)
  echo "  Installing Mobius skills..."
  claude plugin marketplace add tabtoyou/mobius 2>/dev/null || true
  claude plugin marketplace update mobius 2>/dev/null || true
  if claude plugin install mobius@mobius 2>/dev/null; then
    echo "  Skills: installed"
  else
    echo "  Skills: skipped (install manually: claude plugin marketplace add tabtoyou/mobius && claude plugin install mobius@mobius)"
  fi
fi
...
echo '    > mob interview "your idea here"'
echo
echo "  Or from the terminal:"
echo '    mobius init start "your idea here"'
