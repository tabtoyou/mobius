...
Thank you for your interest in contributing to Mobius! This guide covers everything you need to get started.
...
```bash
git clone https://github.com/tabtoyou/mobius && cd mobius
uv sync
uv run mobius --version   # verify
...
- Check [GitHub Issues](https://github.com/tabtoyou/mobius/issues) for open tasks
...
5. **Environment**: Python version, OS, `uv run mobius --version`
...
|-----------|-----------------|
| `src/mobius/cli/commands/` | All user-facing CLI commands and flags |
| `src/mobius/orchestrator/` | Orchestrator runtime, session management, parallel execution |
| `src/mobius/config/` | Configuration schema and defaults |

---

### CLI Commands → Doc Mapping

Any change to a file under `src/mobius/cli/commands/` requires reviewing and updating the corresponding documentation:

#### `init.py` — `mobius init` / `mobius init start`
...
- `docs/getting-started.md` — introductory `mob init` / `mobius init` examples
...
- `docs/getting-started.md` — `mob run` / `mobius run` examples
...
Changes under `src/mobius/orchestrator/` affect runtime behavior documentation:
...
Changes under `src/mobius/config/` affect configuration reference documentation:
...
**`config/loader.py`**: If the config file search path, environment variable names (e.g., `MOBIUS_CONFIG`), or YAML loading logic change, update:
...
Changes under `src/mobius/evaluation/` affect:
...
Changes under `src/mobius/tui/` that alter the visible interface or user interactions affect:
...
Changes under `skills/` (YAML skill definitions used by Claude and Codex) or `src/mobius/plugin/` affect:

| Source path | Must update |
|-------------|-------------|
| `skills/codex.md` | `docs/runtime-guides/codex.md` — if skill instructions change |
| `skills/*.yaml` or `src/mobius/agents/*.md` | `docs/` guide that describes the affected skill/agent behaviour |
| `src/mobius/plugin/skills/executor.py` | `docs/architecture.md` — skill execution model |
| `src/mobius/plugin/agents/registry.py` | `docs/architecture.md` — agent registry; `docs/runtime-capability-matrix.md` if supported agents change per runtime |
...
| `docs/getting-started.md` omits `uv sync` before `uv run mobius` | Critical | User's first command fails with ModuleNotFoundError |
...
| `MOBIUS_AGENT_RUNTIME` written as `MOBIUS_RUNTIME_BACKEND` in one file | High | User sets the wrong env var and the setting silently has no effect |
| Docs recommend `export MOBIUS_MAX_PARALLEL=2` but the variable does not exist | High | User sets the variable; parallelism is not actually limited (false expectation) |
...
| An alternative invocation (`mobius tui` bare vs `mobius tui monitor`) absent | Low | User can use the documented form; no incorrect outcome |
...
1. **Flag parity**: Run `mobius <cmd> --help` and compare every flag to `docs/cli-reference.md`. Any mismatch is a documentation bug.
...
4. **Config key drift**: After any change to `src/mobius/config/models.py`, grep for the changed key name across `docs/` to find stale references.
...
7. **Orchestrator new file**: If a new `.py` file is added to `src/mobius/orchestrator/`, add it to the Orchestrator → Doc Mapping table above before the PR is merged.

```bash
# Quick doc-drift scan: compare CLI help output with cli-reference.md
uv run mobius init --help
uv run mobius run workflow --help
uv run mobius config --help
uv run mobius status --help
...
grep -rn "BINDINGS" src/mobius/tui/screens/ | grep -v "__pycache__"
...
- **GitHub Issues**: [Report bugs or request features](https://github.com/tabtoyou/mobius/issues)
- **GitHub Discussions**: [Ask questions or share ideas](https://github.com/tabtoyou/mobius/discussions)
...
By contributing to Mobius, you agree that your contributions will be licensed under the [MIT License](LICENSE).
