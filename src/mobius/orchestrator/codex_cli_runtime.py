"""Codex CLI runtime for Mobius orchestrator execution."""
...
from mobius.orchestrator.adapter import AgentMessage, RuntimeHandle, TaskResult
...
_SKILL_COMMAND_PATTERN = re.compile(
    r"^\s*(?:(?P<mob_prefix>mob)\s+(?P<mob_skill>[a-z0-9][a-z0-9_-]*)|"
    r"(?P<slash_prefix>/mobius:)(?P<slash_skill>[a-z0-9][a-z0-9_-]*))"
    r"(?:\s+(?P<remainder>.*))?$",
    re.IGNORECASE,
)
...
class CodexCliRuntime:
    ...
    _tempfile_prefix = "mobius-codex-"
    _skills_package_uri = "packaged://mobius.codex/skills"
    ...
    def _compose_prompt(
        ...
        clone_tool = next(
            (
                ...
                if "mobius_clone_decide" in tool or "mobius_clone_decide" in tool
            ),
            None,
        )
        if clone_tool:
            parts.append(
                "## Clone-In-The-Loop Protocol\n"
                f"When you hit an underspecified implementation choice, call `{clone_tool}` "
                "before making the decision yourself. If the clone requests human feedback, "
                "surface that question once, but do not block Ralph loop waiting for a reply. "
                "If no human reply arrives within 5 minutes, use the clone timeout fallback "
                "instead of guessing. If the current runtime cannot actually wait asynchronously, "
                "record the question and continue with the timeout fallback."
            )
    ...
    def _get_builtin_mcp_handlers(self) -> dict[str, Any]:
        """Load and cache local Mobius MCP handlers for exact-prefix dispatch."""
        if self._builtin_mcp_handlers is None:
            ...
            self._builtin_mcp_handlers = {
                ...
                for handler in get_mobius_tools(
    ...
    def _resolve_skill_intercept(self, prompt: str) -> SkillInterceptRequest | None:
        ...
        skill_name = (match.group("mob_skill") or match.group("slash_skill") or "").lower()
        if not skill_name:
            return None

        command_prefix = (
            f"mob {skill_name}" if match.group("mob_skill") is not None else f"/mobius:{skill_name}"
        )
        try:
    ...
    def _build_child_env(self) -> dict[str, str]:
        ...
        Strips ``MOBIUS_AGENT_RUNTIME`` and ``MOBIUS_LLM_BACKEND`` so
        that a child Codex process does not re-load the Mobius MCP server,
        preventing the recursive startup loop described in #185.
        """
        env = os.environ.copy()
        # Prevent child from re-entering Mobius MCP
