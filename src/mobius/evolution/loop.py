...
from mobius.evolution.reflect import ReflectEngine, ReflectOutput
from mobius.evolution.wonder import WonderEngine, WonderOutput
from mobius.observability.drift import DriftMeasuredEvent, DriftMeasurement
...
class EvolutionaryLoopConfig:
    ...
    generation_timeout_seconds: int = int(
        os.environ.get("MOBIUS_GENERATION_TIMEOUT", "0")
    )  # 0 = no timeout
...
class EvolutionaryLoop:
    ...
    ) -> Result[EvolutionaryResult, MobiusError]:
    ...
    async def _run_loop(
        ...
        if not completed_results and not has_interrupted:
            return Result.err(MobiusError("No generations completed before failure"))
    ...
    async def evolve_step(
        ...
        if not events:
            ...
            if lineage is None:
                return Result.err(MobiusError("Failed to project lineage from events"))

            # Check if lineage is already terminated
            if lineage.status in (LineageStatus.CONVERGED, LineageStatus.EXHAUSTED):
                return Result.err(
                    MobiusError(f"Lineage already terminated with status: {lineage.status.value}")
                )
            ...
            if initial_seed is not None:
                ...
                if last_completed is None:
                    ...
                    return Result.err(
                        MobiusError("Events exist but no completed generations found")
                    )
                if last_completed.seed_json:
                    try:
                        ...
                        return Result.err(
                            MobiusError(f"Failed to reconstruct seed from seed_json: {e}")
                        )
                else:
                    return Result.err(
                        MobiusError(
                ...
                return Result.err(MobiusError("Events exist but no completed generations found"))
    ...
    ) -> Result[GenerationResult, MobiusError]:
            ...
            if reflect_output and _should_skip("seeding"):
                ...
                if self.seed_generator:
                    ...
                    if seed_result.is_err:
                        ...
                        return Result.err(
                            MobiusError(f"Seed generation failed: {seed_result.error}")
                        )
        ...
        if execution_output and _should_skip("executing"):
            ...
            try:
                ...
                if hasattr(exec_result, "is_ok") and exec_result.is_ok:
                    ...
                    return Result.err(MobiusError(f"Execution failed: {exec_result.error}"))
                else:
                ...
                return Result.err(MobiusError(f"Execution error: {e}"))
    ...
    async def rewind_to(
        ...
        lineage: OntologyLineage,
        generation_number: int,
    ) -> Result[OntologyLineage, MobiusError]:
        ...
        try:
            ...
            return Result.err(MobiusError(str(e)))
