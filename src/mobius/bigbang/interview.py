...
from mobius.core.file_lock import file_lock as _file_lock
...
class InterviewEngine:
    ...
    state_dir: Path = field(default_factory=lambda: Path.home() / ".mobius" / "data")
    ...
    async def start_interview(
        ...
        if cwd:
            from mobius.bigbang.explore import detect_brownfield
    ...
    def _build_system_prompt(self, state: InterviewState) -> str:
        ...
        preferred_web_tool = os.environ.get("MOBIUS_WEB_SEARCH_TOOL", "").strip()
        web_search_hint = (
    ...
    def _build_ambiguity_snapshot_prompt(self, state: InterviewState) -> str:
        ...
        from mobius.bigbang.ambiguity import AMBIGUITY_THRESHOLD
