...
fn print_help() {
    eprintln!("mobius-tui — Rust TUI dashboard for Mobius");
    eprintln!();
    eprintln!("USAGE:");
    eprintln!("  mobius-tui [monitor] [OPTIONS]");
    eprintln!();
    eprintln!("OPTIONS:");
    eprintln!("  --db-path <PATH>   Path to mobius.db (default: ~/.mobius/mobius.db)");
    eprintln!("  --mock             Use demo data instead of DB");
    eprintln!("  --help, -h         Show this help");
    eprintln!();
    eprintln!("EXAMPLES:");
    eprintln!("  mobius-tui                     # default DB");
    eprintln!("  mobius-tui monitor             # same as above");
    eprintln!("  mobius-tui --mock              # demo mode");
    eprintln!("  mobius-tui --db-path /tmp/o.db # custom DB path");
}
...
fn main() -> std::io::Result<()> {
    ...
    let db_path = args
        ...
        .unwrap_or_else(|| {
            ...
            PathBuf::from(home).join(".mobius/mobius.db")
        });

    let mut state = AppState::new();
    let mut ouro_db: Option<db::MobiusDb> = None;
    ...
    if use_mock {
        ...
        match db::MobiusDb::open(&db_path) {
            Ok(mut conn) => {
