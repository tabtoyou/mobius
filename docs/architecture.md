...
**In the normal flow, seeds are auto-generated by the Socratic interview** (`mob interview` in Claude Code, or via MCP tools). Most users never need to create or edit a seed manually — the interview handles crystallization automatically.
...
- **`ClaudeAgentAdapter`** (`backend="claude"`) — Wraps Claude Agent SDK / Claude Code CLI with streaming, retry, and session resumption. Module: `src/mobius/orchestrator/adapter.py`
- **`CodexCliRuntime`** (`backend="codex"`) — Drives the OpenAI Codex CLI as a session-oriented runtime with NDJSON event parsing. Module: `src/mobius/orchestrator/codex_cli_runtime.py`
...
- **Server mode** (`mobius mcp serve`) — Exposes tools (`mobius_execute_seed`, `mobius_session_status`, `mobius_query_events`) to Claude Desktop and other MCP clients
- **Client mode** (`mobius run --mcp-config mcp.yaml`) — Discovers and consumes tools from external MCP servers (filesystem, GitHub, databases, etc.), merged with built-in tools
...
- **Agents** — Add bundled specialist prompts in `src/mobius/agents/`; use `MOBIUS_AGENTS_DIR` for explicit local overrides
- **MCP integration** — Bidirectional: expose Mobius tools as an MCP server, or consume external MCP servers during execution
