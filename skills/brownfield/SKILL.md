...
```
mob brownfield                # Scan repos and set defaults
mob brownfield scan           # Scan only (no default selection)
mob brownfield defaults       # Show current defaults
mob brownfield set 6,18,19   # Set defaults by repo numbers
```
...
### Default flow (`mob brownfield` with no args)
...
You can set defaults anytime with: mob brownfield
```
...
```
No default repos set. Run 'mob brownfield' to configure.
```
